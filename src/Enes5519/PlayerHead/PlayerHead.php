...
class PlayerHead extends PluginBase implements Listener
{
	 ...
	 * @param int $num
	 */
	private function addGet(Player $p,$num): void
	{
		$n = $p->getName();
		if ($this->data->exists($n))
		{
			...
			$data[$num] = $num;
			$p->sendMessage(ReefAPI::GOOD."プレゼントを見つけた あと6個見つけよう");
		}
	...
	private function getChristmas(int $num): Item
	{
		...
		return (ItemFactory::get(Item::MOB_HEAD, 3))
			->setCustomBlockData(new CompoundTag('Skin', [
				new StringTag('Name', "$num"),
				new ByteArrayTag('Data', $data),
				new IntTag(self::Christmas, $num),
			]))
			->setCustomName(TextFormat::colorize(sprintf(self::$headFormat, self::Christmas . ' number ' . $num), '&'));
	}
}